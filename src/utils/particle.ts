...
/** 初始化配置控制器 */
export function particleInitController(
	particleItem: ParticleItem,
	_status: CallbackStatusParam,
	...
	if (children?.length) {
		...
		parent
			? reactElementsRef.current.children[`${parent}-children`]!.push(element)
			: (reactElementsRef.current.element[key] = element)
	} else {
		const element = createElement(Updater, {
			particleCmpt: registered,
			particleProps: props,
...
/** 新增（append）控制器 */
export function particleAppendController(
	particleItem: ParticleItem,
	status: CallbackStatusParam,
	controllerExtra: ControllerExtra
) {
	// 将新增配置更新到树中
...
/** 移除(remove)控制器 */
export function particleRemoveController(
	_particleItem: ParticleItem,
	status: CallbackStatusParam,
	controllerExtra: ControllerExtra
) {
	const { operationKey } = status
...
/** 属性设置（setItem）控制器 */
export function particleSetItemController(
	particleItem: ParticleItem,
	status: CallbackStatusParam,
	controllerExtra: ControllerExtra
) {
...
/** 配置替换（replace）控制器 */
export function particleReplaceController(
	particleItem: ParticleItem,
	status: CallbackStatusParam,
	controllerExtra: ControllerExtra
) {
